...
export default function Tasks() {
  ...
  return (
    <section className="content-shell">
      ...
      <PageHeader
        ...
        action={(
          <div className="toolbar">
            <div className="quick-sort-pills" role="tablist" aria-label="Tasks sort">
              <button className={`ghost ${taskScope === 'team' ? 'active-view' : ''}`} type="button" onClick={() => setTaskScope('team')}>All Team Tasks</button>
              <button className={`ghost ${taskScope === 'mine' ? 'active-view' : ''}`} type="button" onClick={() => setTaskScope('mine')}>My Tasks</button>
            ...
            {createAllowed ? <button className="primary" type="button" onClick={() => setCreateOpen(true)}>Create Task</button> : null}
            <button className="secondary" type="button" onClick={() => load(filters, taskScope)}>Refresh</button>
          </div>
        )}
      />
      ...
      {viewMode === 'board' ? (
        ...
        <div className="panel table-wrap">
          <table>
            <thead>
              <tr>
                <th><button type="button" className="table-sort-btn" onClick={() => toggleSort('title')}>Title {sortMark('title')}</button></th>
                <th><button type="button" className="table-sort-btn" onClick={() => toggleSort('project')}>Project {sortMark('project')}</button></th>
                <th>Team</th>
                <th><button type="button" className="table-sort-btn" onClick={() => toggleSort('assignee')}>Assignee {sortMark('assignee')}</button></th>
                ...
                <th><button type="button" className="table-sort-btn" onClick={() => toggleSort('dueDate')}>Due Date {sortMark('dueDate')}</button></th>
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={task.id} className={overdue ? 'overdue-row' : ''}>
                    ...
                    <td data-label="Actions">
                      <div className="table-actions">
                        <button className="ghost task-icon-btn" type="button" title="View" aria-label="View task" onClick={() => openDetails(task)}>
                        ...
                        {canManage ? (
                          <button className="ghost task-icon-btn" type="button" title="Edit" aria-label="Edit task" onClick={() => openEdit(task)}>
                        ...
                        {isAdmin ? (
                          <button className="ghost danger-btn task-icon-btn" type="button" title="Delete" aria-label="Delete task" onClick={() => setTaskToDelete(task)}>
      ...
      <Drawer open={isFilterOpen} title="Task Filters" onClose={() => setFilterOpen(false)}>
        <div className="stack">
          <label>Search
            <input value={filterDraft.search} onChange={(e) => setFilterDraft({ ...filterDraft, search: e.target.value })} placeholder="Search task title" />
          </label>
          <label>Team
