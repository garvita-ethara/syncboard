...
export default function App() {
  const { token, user, booting, logout, reloadUser } = useAuth();
  ...
  async function handlePresenceChange(nextPresence) {
    try {
      await apiRequest('/users/me/presence', { token, method: 'PATCH', body: { presence: nextPresence } });
      await reloadUser(token);
      if (typeof window !== 'undefined' && user?.id) {
        window.dispatchEvent(new CustomEvent('presence:changed', {
          detail: { userId: user.id, presence: nextPresence }
        }));
      }
      toast?.pushToast({ type: 'success', title: 'Status updated', message: `You are now ${String(nextPresence).toLowerCase()}.` });
    } catch (error) {
      toast?.pushToast({ type: 'error', title: 'Status update failed', message: error.message || 'Please try again.' });
    }
  }
  ...
  const pageTitle = nav.find(([path]) => path === pathname)?.[1]
    || (pathname === '/users' ? 'Users' : pathname === '/unauthorized' ? 'Unauthorized' : 'Dashboard');
          ...
          <div className="app-shell">
            ...
            <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
              ...
              <div className="sidebar-footer">
                <ProfileMenu
                  user={user}
                  onSetPresence={handlePresenceChange}
                  onLogout={handleLogout}
            ...
            <div className="workspace">
              <Navbar onMobileMenu={() => setSidebarOpen(true)} title={pageTitle} />
